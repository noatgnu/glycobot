...
def get_chemrxiv(args):
    ...
    a = []
    for n, i in enumerate(p.search(':title: glycosylation or lection or carbohydrate or glycoprotein or glycan or glycosyltransferase or sialic :description: glycosylation or lection or carbohydrate or glycoprotein or glycan or glycosyltransferase or sialic', max_page=args.max, break_entry=args.sc)):
        if len(i) > 0:
            if n == 0:
                stop_art = i[0].id
            a += i
